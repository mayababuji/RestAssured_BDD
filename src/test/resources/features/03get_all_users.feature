Feature: User Management API - Get All Users
  ...
  Scenario: Verify request with case sensitive "Accept: APPLICATION/JSON" returns JSON
    Given Admin sets get request with valid credentials
    When Admin sets GET request with Accept header as "APPLICATION/JSON"
    Then The response content type should be "application/json"

  Scenario: Verify custom header like X-Request-ID and confirm it does not break functionality
    Given Admin sets get request with valid credentials
    When Admin sets GET request with custom header as "application/xml"
    Then The response content type should be "application/json"

  Scenario: Verify sending POST request to GET /users endpoint returns 405 error
    Given Admin sets get request with valid credentials
    When Admin sends POST request to  endpoint
    Then The response status code should be "405"

  Scenario: Verify  Get All User List with invalid endpoint does not retrieve any value
    Given Admin sets get request with valid credentials
    When Admin sends HTTPS Request with invalid endpoint
    Then The response status code should be "404"

  Scenario: Verify  Get  all user with No Auth
    Given Admin set the get request without Auth
    When Admin sends a GET request to endpoint
    Then The response status code should be "401"

  Scenario: Verify Get  all user with Invalid Basic Auth
